        ...
        m_playbackWorker(playbackWorker)
{
    ...
    for (const auto& videoFile : videoFiles) {
        ...
        auto windowPtr = videoFile.windowPtr;
        qDebug() << "Created and showed VideoWindow for index" << fc_index << "API" << static_cast<int>(videoFile.graphicsApi);

        windowPtr->initialize(metaPtr);
        qDebug() << "Renderer initialized for index" << fc_index;

        auto frameController = std::make_unique<FrameController>(nullptr, std::move(decoder), playbackWorker, windowPtr, fc_index);
        ...
        connect(windowPtr, &VideoWindow::batchUploaded, this, &VideoController::uploadReady, Qt::AutoConnection);
