...
AVRational Timer::forwardWake() {
    auto min = std::min_element(m_timestamp.begin(), m_timestamp.end(), [](const AVRational& a, const AVRational& b) {
...
AVRational Timer::backwardWake() {
    auto max = std::max_element(m_timestamp.begin(), m_timestamp.end(), [](const AVRational& a, const AVRational& b) {
        return av_cmp_q(a, b) < 0;
    });
    return *max;
}
